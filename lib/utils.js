...
// (`public/locales/en/a/b.json`) are a documented i18next pattern, so
// rejecting them would be an unintended regression (see issue #74).
// Directory escape is still prevented — `..` is still blocked, `\` is still
// blocked, and the security fix remains in force for every concrete attack
// pattern from the original advisory.
export function isSafeNsSegment (v) {
  return isSafeSegmentBase(v)
}

// Backwards-compatible alias for the strict check.
